...
// globals
var schemas = [	// currently hard-coded but will ultimate come from schema server
		{
	...
	try{
		...
		log.message(log.DEBUG, "endpoint: " + endpoint);
			...
			if(token[req.method]){
				...
				switch(req.method){
					case("POST"):
						// todo: POST returns an HTTP Status and a token with POST, GET, PUT, DELETE and OWNER
					...
					case("GET"):
						...
						if(token.object_id === obj.object_id){
							...
							if(url.parse(req.url,true).query.new_token){
								...
								token_permissions.POST = url.parse(req.url,true).query.POST || false;
								token_permissions.GET = url.parse(req.url,true).query.GET || false;
								token_permissions.PUT = url.parse(req.url,true).query.PUT || false;
								token_permissions.DELETE = url.parse(req.url,true).query.DELETE || false;
